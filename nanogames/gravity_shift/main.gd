...
func _spawn_obstacle() -> void:
	...
	match _difficulty:
		1:
			obstacle = _OBSTACLE_EASY.instance()
		2:
			obstacle = _OBSTACLES_MEDIUM[\
					randi() % _OBSTACLES_MEDIUM.size()].instance()
		3:
			obstacle = _OBSTACLES_HARD[\
					randi() % _OBSTACLES_HARD.size()].instance()

	add_child(obstacle)
	obstacle.position = ($ObstaclePosition as Position2D).position
