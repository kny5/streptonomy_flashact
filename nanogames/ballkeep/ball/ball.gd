...
func _physics_process(delta: float) -> void:
	...
	_speed = _speed.bounce(collision.normal)

	($Spark as Particles2D).global_position = collision.position
	($Spark as Particles2D).emitting = true

	($Bounce as AudioStreamPlayer2D).play()


func direct_to(rotation: float) -> void:
	_has_bounced_once = false

	_speed = Vector2(0, SPEED_MAX * SPEED_START_BASE).rotated(rotation)
