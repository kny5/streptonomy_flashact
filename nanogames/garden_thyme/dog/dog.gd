...
func enable_walk(movement_area: Rect2) -> void:
	($Sprite as Sprite).frame = 1

	($Sleep as Particles2D).restart()
	($Sleep as Particles2D).emitting = false
...
func _on_area_entered(_area: Area2D):
	...
	($Sprite as Sprite).flip_h = _direction_speed.x < 0\
