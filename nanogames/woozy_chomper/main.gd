...
func nanogame_prepare(difficulty: int, debug_code: int) -> void:
	if difficulty > 1:
		...
		if difficulty == 3:
			($ArrowUp as Sprite).hide()
			($ArrowDown as Sprite).hide()
			($ArrowLeft as Sprite).hide()
			($ArrowRight as Sprite).hide()

			($Edges/WallUp as TextureRect).show()
			($Edges/WallDown as TextureRect).show()
			($Edges/WallLeft as TextureRect).show()
			($Edges/WallRight as TextureRect).show()
...
func _queue_fruit() -> void:
	...
	var fruit := Fruit.instance() as Area2D
	# Make the fruit appear above the creature's segments for visibility.
	fruit.z_index = 1
	add_child(fruit)
	...
	var segments: Array = woozy.get_segments()
	...
	while true:
		...
		if not _fruit_queue.empty() and spawn_position.distance_to(
