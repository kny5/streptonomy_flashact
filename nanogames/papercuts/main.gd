...
func _unhandled_input(event: InputEvent) -> void:
	if not _is_cutting or not event is InputEventMouse:
		return
...
func nanogame_prepare(difficulty: int, _debug_code: int) -> void:
	...
	var shape_index: int = randi() % SHAPE_SECTION_SIZE if _debug_code == 0\
			else int(clamp(_debug_code, 1, SHAPE_SECTION_SIZE)) - 1
	...
	($StartIndicator as Sprite).position = _cut_position_start

	($ScissorsIndicator as Sprite).position = _cut_position_start
	($ScissorsIndicator as Sprite).rotation =\
	...
	_area_inner_points =\
			Geometry.offset_polygon_2d(shape.polygon, -POLYGON_OFFSET).front()
	# Avoid using `first()` to get the first element, as it causes a crash on
...
func _miscut() -> void:
	...
	var scissors_indicator := $ScissorsIndicator as Sprite
...
func _get_polygon_size(polygon: Array) -> float:
	var count = polygon.size() - 1
	var size := 0.0
	for i in count:
...
func _on_AreaValid_input_event(
	...
	if not _is_cutting and event.button_mask == BUTTON_LEFT:
		...
		($StartIndicator as Sprite).show()

		($ScissorsIndicator as Sprite).hide()
		($AnimationPlayer as AnimationPlayer).stop()
	elif event.button_mask != BUTTON_LEFT:
		if _is_cutting:
			_is_cutting = false

			var scissor_indicator := $ScissorsIndicator as Sprite
	...
	if event.position.distance_to(_cut_position_start) <= INDICATOR_RADIUS and\
			Geometry.clip_polygons_2d(
					_area_inner_points, _cut_marks.points).empty():
		_is_cutting = false

		var polygon_main: Array = _cut_marks.points
		if not Geometry.is_polygon_clockwise(polygon_main):
			polygon_main.invert()
		...
		while polygon_index < polygon_main.size() - 1:
			for j in range(polygon_index + 2, polygon_main.size() - 1):
				var intersection = Geometry.segment_intersects_segment_2d(
						polygon_main[polygon_index],
						polygon_main[polygon_index + 1], polygon_main[j],
						polygon_main[j + 1])
				if intersection != null:
					var polygon: Array = polygon_main.slice(polygon_index, j)
	...
	if event.position.distance_to(_cut_position_current) > CUT_DISTANCE:
		...
		for i in points.size() - 1:
			if Geometry.segment_intersects_segment_2d(_cut_position_current,
