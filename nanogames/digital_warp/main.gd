...
func _spawn_obstacle() -> void:
	...
	var past_rotations := []
	for i in _difficulty:
		var obstacle := Obstacle.instance() as Area2D
		add_child(obstacle)
		...
		if i == 0:
			travel_rotation = wrapf(($Spaceship as Area2D).rotation +\
					rand_range(-OBSTACLE_DISTANCE, OBSTACLE_DISTANCE), 0, TAU)

			# warning-ignore:return_value_discarded
			obstacle.connect("traveled", self, "_spawn_obstacle")
		else:
			while true:
				travel_rotation = TAU * randf()

				var is_colliding := false
				for j in past_rotations:
					if abs(travel_rotation - j) < OBSTACLE_DISTANCE:
						is_colliding = true

						break

					var min_rotation_decreased: float =\
							min(travel_rotation, j) - OBSTACLE_DISTANCE
